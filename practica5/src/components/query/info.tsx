...
const InfoCity: FC<CityProps> = (props) => {
  const { data, loading, error } = useQuery<ICity>(City, {
  ...
  return (
    <div className="container_global">
      ...
      {data &&
        data.cities.map((city, index_city) => {
          if (index === index_city) {
            return (
              <div className="container_info">
                ...
                <div className="args">Población: {city.population}</div>
                {city.timeZone && (
                  <div className="args">
                    Franja Horaria: {city.timeZone.name}
                  </div>
                )}
              </div>
            );
